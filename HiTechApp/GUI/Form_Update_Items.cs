...
{
    public partial class Form_Update_Items : Form
    {
        public Form_Update_Items()
        {
            InitializeComponent();
        }
        ...
        private void Form_Update_Items_Load(object sender, EventArgs e)
        {
            reset_inputs();
        }
        ...
        private void btn_update_Click(object sender, EventArgs e)
        {
            if (cbo_itemtype.Text == "Book")
            {
                if (txt_title.Text != "" && Validators.Valid_digit(txt_price.Text) != "" && Validators.Valid_10_Digits(txt_isbn10.Text.Trim()) != "" && Validators.Valid_13_Digits(txt_isbn13.Text) != "" && Validators.Valid_digit(txt_quantity.Text) != "" && lbl_authors.Text.Trim() != "")
                {
                    ...
                    book_list();
                    reset_inputs();
                    MessageBox.Show("Update Completed. Please see the list below.");
                }
                else
                {
        ...
        private void cbo_itemtype_SelectedIndexChanged_1(object sender, EventArgs e)
        {
        ...
        private void listView1_DoubleClick(object sender, EventArgs e)
        {
            ...
            if (type == "Book")
            {
                lbl_id.Text = one.Searched_Book_by_ID(id).ID;
                txt_title.Text = one.Searched_Book_by_ID(id).Title;
                cbo_category.Text = one.Searched_Book_by_ID(id).Category;
                string[] aid = one.Searched_Book_by_ID(id).AuthorID.Split(',');
                foreach (string i in aid)
                {
                    lbl_authors.Text += " " + i; 
                }
                cbo_pubyear.Text = one.Searched_Book_by_ID(id).Pub_Year;
                cbo_publisher.Text = one.Searched_Book_by_ID(id).PubID + " : " + one.Searched_publisher_by_id(one.Searched_Book_by_ID(id).PubID).Pub_Name;
                txt_price.Text = one.Searched_Book_by_ID(id).UnitPrice.ToString();
                cbo_itemtype.Text = one.Searched_Book_by_ID(id).Type;
                txt_isbn10.Text = one.Searched_Book_by_ID(id).ISBN10;
                txt_isbn13.Text = one.Searched_Book_by_ID(id).ISBN13;
                txt_quantity.Text = one.Searched_Book_by_ID(id).Quantity.ToString();
            }
            if (type == "Software")
            {
                lbl_id.Text = one.Searched_Software_by_ID(id).ID;
                txt_title.Text = one.Searched_Software_by_ID(id).Title;
                cbo_category.Text = one.Searched_Software_by_ID(id).Category;
                string[] aid = one.Searched_Software_by_ID(id).AuthorID.Split(',');
                foreach (string i in aid)
                {
                    lbl_authors.Text += " " + i;
                }
                cbo_pubyear.Text = one.Searched_Software_by_ID(id).Pub_Year;
                cbo_publisher.Text = one.Searched_Software_by_ID(id).PubID + " : " + one.Searched_publisher_by_id(one.Searched_Software_by_ID(id).PubID).Pub_Name;
                txt_price.Text = one.Searched_Software_by_ID(id).UnitPrice.ToString();
                cbo_itemtype.Text = one.Searched_Software_by_ID(id).Type;
                cbo_s_type.Text = one.Searched_Software_by_ID(id).Software_Type;
                txt_quantity.Text = one.Searched_Software_by_ID(id).Quantity.ToString();
            }
        }
        ...
        private void cbo_publisher_MouseClick(object sender, MouseEventArgs e)
        {
